...
export default function Profile() {
  ...
  const background_div = "bg-white kufam-font bg-opacity-10 backdrop-blur-sm shadow-lg px-6 py-2 rounded-lg hover:shadow-2xl hover:bg-opacity-16 transition-shadow duration-300";
  const sections = "m-0 p-0"
  useEffect(() => {
  ...
  return (
    <main className='flex text-white px-2 w-full py-2 h-full justify-center' style={{
    ...
    {isNewCardOpen && (
      ...
      <section className={`left space-y-2 pl-10 ${sections}`}>

        <div className='flex space-x-1 pl-5'>
          <span className='pt-1'>
            <svg width="48" height="48" viewBox="0 0 72 74" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <h1 className='text-2xl'>Welcome <h2 className='text-xl'>Placeholder</h2></h1>
        </div>
      ...
      <section className="middle flex flex-col items-center justify-center w-full space-y-4">
        <nav className={`${background_div} p-0 mb-2`}>
          ...
          <Button onClick={handleAnalyze} className={`p-0 m-2 text-xl text-white bg-opacity-0`}>Analyze Card Spending</Button>
          <Button onClick={handleTransactions} className={`p-0 m-2 text-xl text-white bg-opacity-0`}>Transactions</Button>  
        </nav>
        {
          cardSection === "analyze" ? (
            <>
      ...
      <section className={`right mr-5 ${sections}`} id='right'>
        <Chat />
      </section>
    </main>
  );
